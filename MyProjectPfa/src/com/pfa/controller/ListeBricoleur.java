...
/**
 * Servlet implementation class ListeBricoleur
 */
@WebServlet("/ListeBricoleur")
public class ListeBricoleur extends HttpServlet {
	private static final long serialVersionUID = 1L;

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public ListeBricoleur() {
		super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (request.getParameter("id") != null) {
			...
			List<Disponibilite> liste = dao.getAllServiceId(id);
			request.setAttribute("liste", liste);

		} else if (request.getParameter("search") != null) {
			String id = request.getParameter("search");
			List<Disponibilite> liste = dao.Recherche(id);
			request.setAttribute("liste", liste);

		} else {
			List<Disponibilite> liste = dao.getAllDisponibilite();
			request.setAttribute("liste", liste);
		...
		request.getRequestDispatcher("listebricoleur.jsp").forward(request, response);
